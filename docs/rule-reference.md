...
| **Answer Option Formatting** | `prompts-answer-option-format` | warning | ai | ✗ | — | Uses AI to check answer-choice capitalization and terminal punctuation. Phrase options that complete a fill-in-the-blank stem should start lowercase; for standalone questions, leading capitals are accepted. A terminal period is expected only on full-sentence options. |
...
| **Italicize biological nomenclature** | `bio-italics-nomenclature` | warning | ai | ✗ | — | Uses AI to check that genus/species names, gene symbols, and restriction-enzyme prefixes are italicized, and that protein names are not. |
